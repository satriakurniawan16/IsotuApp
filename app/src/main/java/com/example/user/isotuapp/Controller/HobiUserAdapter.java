...
public class HobiUserAdapter  extends RecyclerView.Adapter<HobiUserAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        ...
        if(pet.getIduser() != null ){
            final DatabaseReference user = FirebaseDatabase.getInstance().getReference("user").child(pet.getIduser());
            user.addValueEventListener(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    User user1 = dataSnapshot.getValue(User.class);
                    if(user1 != null) {
                        holder.nameTextView.setText(user1.getFullname());
                        Picasso.get().load(user1.getImage()).into(holder.profilImageview);
